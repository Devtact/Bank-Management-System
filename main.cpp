...
string suno::up="";
  ...
  void suno::add_acc()
    ...
    bool suno::login()
    {
        ...
        cin>>usern>>pswd;
        query.clear();
        query="select * from emp where username='"+string(usern)+"' and password='"+string(pswd)+"' ";
        qstate= mysql_query(conn,query.c_str());
         res = mysql_store_result(conn);
        while((row=mysql_fetch_row(res)))
        {
        ...
        while((row=mysql_fetch_row(res)))
        {
           ...
           ptemp=string(row[4]);
            ...
            //cout<<usern<<" "<<utemp<<"=="<<pswd<<" "<<ptemp;
            Sleep(500);
        if(count==1 && usern==utemp && pswd==ptemp )
        {
            suno::up=string(usern);
           // cout<<"\n("<<up<<")\n";
            return true;
        }
        else
...
if (a.conn)
    ...
    if(a.login()==true)
       ...
       {
           cout<<"Login Unsuccessful... PRESS \"E\" to Exit";
           if(cin.get()==69 || cin.get()==101)
            exit(0);
           else
           goto nami;
       }
   ...
   do
   {
    system("cls");
    gotoxy(5,0);
    puts("You are an Valuable to Company");
        ...
        switch(a1)
            {
                ...
                case 9: a.up="";goto nami;break;
                case 10: load(); exit(0); break;
        ...
        switch(a1)
            {
                ...
                case 3: a.up="";goto nami;break;
                case 4: load(); exit(0); break;
                default:cout<<"Plz enter Number from Given Option ONLY..";break;
            }
